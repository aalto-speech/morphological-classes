...
double
Exchange::evaluate_split(int class_idx,
                         const set<int> &class1_words,
                         const set<int> &class2_words) const
{
    if (m_classes[class_idx].size() < 2) {
        cerr << "Error, evaluating splitting a class with " << m_classes[class_idx].size() << " words" << endl;
        exit(1);
    }

    int class1_count = 0, class2_count = 0;
    for (auto wit = class1_words.cbegin(); wit != class1_words.cend(); ++wit)
        class1_count += m_word_counts.at(*wit);
    for (auto wit = class2_words.cbegin(); wit != class2_words.cend(); ++wit)
        class2_count += m_word_counts.at(*wit);

    double cc_diff = 2 * m_class_counts[class_idx] * log(m_class_counts[class_idx]);
    if (class1_count != 0) cc_diff -= 2 * (class1_count) * log(class1_count);
    if (class2_count != 0) cc_diff -= 2 * (class2_count) * log(class2_count);

    double bg_diff = 0.0;
    for (int j=0; j<(int)m_class_bigram_counts[class_idx].size(); j++) {
        if (j==class_idx) continue;
        int curr_count = m_class_bigram_counts[class_idx][j];
        if (curr_count == 0) continue;

        int class1_count = curr_count, class2_count = 0;
        for (auto wit = class2_words.begin(); wit != class2_words.end(); ++wit) {
            auto countit = m_word_class_counts[*wit].find(j);
            if (countit == m_word_class_counts[*wit].end()) continue;
            class2_count += countit->second;
            class1_count -= countit->second;
        }
        if (curr_count != 0) bg_diff -= curr_count * log(curr_count);
        if (class1_count != 0) bg_diff += class1_count * log(class1_count);
        if (class2_count != 0) bg_diff += class2_count * log(class2_count);
    }

    for (int i=0; i<(int)m_class_bigram_counts.size(); i++) {
        if (i==class_idx) continue;
        int curr_count = m_class_bigram_counts[i][class_idx];
        if (curr_count == 0) continue;

        int class1_count = curr_count, class2_count = 0;
        for (auto wit = class2_words.begin(); wit != class2_words.end(); ++wit) {
            auto countit = m_class_word_counts[*wit].find(i);
            if (countit == m_word_class_counts[*wit].end()) continue;
            class2_count += countit->second;
            class1_count -= countit->second;
        }
    ...
    int curr_count = m_class_bigram_counts[class_idx][class_idx];
    if (curr_count == 0) return cc_diff + bg_diff;

    int count11=0, count12=0, count21=0, count22=0;
    for (auto wit1=class1_words.begin(); wit1!=class1_words.end(); ++wit1) {
        for (auto wit11=class1_words.begin(); wit11!=class1_words.end(); ++wit11) {
            auto count11it = m_word_bigram_counts[*wit1].find(*wit11);
            if (count11it != m_word_bigram_counts[*wit1].end())
                count11 += count11it->second;
        }
        for (auto wit12=class2_words.begin(); wit12!=class2_words.end(); ++wit12) {
            auto count12it = m_word_bigram_counts[*wit1].find(*wit12);
            if (count12it != m_word_bigram_counts[*wit1].end())
                count12 += count12it->second;
        }
    }

    for (auto wit2=class2_words.begin(); wit2!=class2_words.end(); ++wit2) {
        for (auto wit21=class1_words.begin(); wit21!=class1_words.end(); ++wit21) {
            auto count21it = m_word_bigram_counts[*wit2].find(*wit21);
            if (count21it != m_word_bigram_counts[*wit2].end())
                count21 += count21it->second;
        }
        for (auto wit22=class2_words.begin(); wit22!=class2_words.end(); ++wit22) {
            auto count22it = m_word_bigram_counts[*wit2].find(*wit22);
            if (count22it != m_word_bigram_counts[*wit2].end())
                count22 += count22it->second;
        }
    }

    bg_diff -= curr_count * log(curr_count);
    if (count11 != 0) bg_diff += count11 * log(count11);
    if (count12 != 0) bg_diff += count12 * log(count12);
    if (count21 != 0) bg_diff += count21 * log(count21);
    if (count22 != 0) bg_diff += count22 * log(count22);

    return cc_diff + bg_diff;
}
